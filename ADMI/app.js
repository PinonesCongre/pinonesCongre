...
coleccionInformes.on("child_added", datos => {        
    dataSet = [datos.key, datos.child("nombre").val(),datos.child("rol").val(), datos.child("publicaciones").val(), datos.child("horas").val(), datos.child("revisitas").val(),datos.child("cursosBiblicos").val(),datos.child("videos").val(),datos.child("supervisor").val(),datos.child("notas").val()];
...
$('form').submit(function(e){                         
    ...
    let supervisor = $.trim($('#supervisor').val());
    let notas = $.trim($('#notas').val());                         
    let idFirebase = id;        
    if (idFirebase == ''){                      
        idFirebase = coleccionusers.push().key;
    };                
    data = {nombre:nombre, rol:rol, publicaciones:publicaciones, horas:horas, revisitas:revisitas, cursosBiblicos:cursosBiblicos, videos:videos, supervisor:supervisor, notas:notas};             
    actualizacionData = {};
...
$("#tablaInformes").on("click", ".btnEditar", function() {    
    ...
    let supervisor = $(this).closest('tr').find('td:eq(7)').text();   
